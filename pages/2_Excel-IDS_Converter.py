...
def main(uploaded_file: UploadedFile) -> None:
    with st.container():
        # Create Dataframe
        if uploaded_file:
            st.session_state.convert = False

            st.session_state.df_specifications = pd.read_excel(
                uploaded_file, dtype=str, skiprows=2, sheet_name="SPECIFICATIONS"
            )
            st.session_state.df_applicability = pd.read_excel(
                uploaded_file, dtype=str, skiprows=2, sheet_name="APPLICABILITY"
            )
            st.session_state.df_requirements = pd.read_excel(
                uploaded_file, dtype=str, skiprows=2, sheet_name="REQUIREMENTS"
            )
            ...
            if st.session_state.df_specifications is not None:
                ...
                if submitted:
                    ...
                    for index, spec in st.session_state.df_specifications.iterrows():
                        ...
                        df_app_spec = st.session_state.df_applicability.query(
                            f"specification == '{spec.iloc[0]}'"
                        ).fillna("")
                        df_req_spec = st.session_state.df_requirements.query(
                            f"specification == '{spec.iloc[0]}'"
                        ).fillna("")
                        ...
                        for index, row in df_req_spec.iterrows():
                            ...
                            property = (
                                ids.Property(
                                    ...
                                    maxOccurs="unbounded"
                                    if row["optionality"].upper() in ["REQUIRED", "OPTIONAL"]
                                    else 0,
                                )
                                if row["property name"] != ""
                                else None
                ...
                if st.session_state.convert and st.session_state.ids:
                    st.download_button(
                        "📥 Download IDS file", st.session_state.ids, file_name=st.session_state.file_name, mime="xml"
                    )
